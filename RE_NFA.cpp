...
public:
	...
	void display() {
		...
		noOfStates=transitions[transitions.size()-1].vertex_to+1;
		int god[noOfStates][inputSymbol+1][20];
        //memset(god, -1,noOfStates*inputSymbol*noOfStates * sizeof(int));
        for(int i=0;i<noOfStates;i++){
            for(int j=0;j<inputSymbol+1;j++){
        ...
        ofstream foutt("NFA1.txt");
        foutt<<noOfStates+1<<" "<<inputSymbol<<"\n";
        foutt<<1<<" "<<get_final_state()<<"\n";
	    foutt<<counter<<"\n";
	    flag=0,flag1=0;
		for(int i=0;i<noOfStates;i++)
        {
...
NFA concat(NFA a, NFA b) {
	...
	//result.set_transition(a.get_final_state(), a.get_vertex_count(), '0');
...
NFA kleene(NFA a) {
	...
	result.set_transition(a.get_vertex_count(), a.get_vertex_count() + 1, '0');
	result.set_transition(a.get_vertex_count(), 1, '0');
	result.set_transition(0, a.get_vertex_count() + 1, '0');
...
NFA or_selection(vector<NFA> selections, int no_of_selections) {
	...
	for(i = 0; i < no_of_selections; i++) {
		result.set_transition(0, adder_track, '0');
		med = selections.at(i);
		...
		result.set_transition(adder_track - 1, vertex_count - 1, '0');
	}
...
int main() {
    ...
    cout<<endl<<"Enter the number of input symbols [1,2,...n]";
    cin>>inputSymbol;
